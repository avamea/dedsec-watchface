...
static void battery_handler(BatteryChargeState charge_state) { 
	...
	if(charge_state.charge_percent<=25){
		#ifdef PBL_COLOR
		  text_layer_set_background_color(s_connection_layer, GColorDarkCandyAppleRed);
		  text_layer_set_background_color(s_connection_bg_layer, GColorBulgarianRose);
		#endif
	}else{
		#ifdef PBL_COLOR
		  text_layer_set_background_color(s_connection_layer, GColorCyan);
...
static void main_window_load(Window *window) {
	...
	s_connection_bg_layer = text_layer_create(GRect(0, 123, 122, 18));	
  	...
  	text_layer_set_text(s_connection_bg_layer, " CONNECTION IS POWER");
	...
	s_connection_layer = text_layer_create(GRect(0, 123, 122, 18));	
	  ...
	  text_layer_set_background_color(s_connection_layer, GColorCyan);
	#else
	  text_layer_set_background_color(s_connection_layer, GColorWhite);
	#endif  	
  	text_layer_set_text_color(s_connection_layer, GColorBlack);
  	text_layer_set_text(s_connection_layer, " CONNECTION IS POWER");
